...
SEXP do_windowsextension(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    int nprotect = 0;
...
SEXP do_unixextension(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    int nprotect = 0;
...
SEXP do_extension(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    int windows = asLogical(eval(install("os.windows"), rho));
    if (windows == NA_LOGICAL)
        error("invalid 'os.windows'; should never happen, please report!");
    if (windows)
        return do_windowsextension(call, op, args, rho);
    else return do_unixextension(call, op, args, rho);
}
